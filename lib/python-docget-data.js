      ...
      res.on('end', () => {
        ...
        while(flag){
          ...
          if(parsedtag['type'] == 'h2' && 'id' in parsedtag['tags']){
            ...
            tagendindex = index
            endindex = this.matchTag(rawData,beginindex)

            while(rawData[index]!="<"){ index+=1; }
            let nexttagindex = index
            while(rawData[index]!=">"){ index+=1; }
            index+=1

            let nexttag = this.parseTag(rawData.substring(nexttagindex,index))
            if(nexttag['type'] == 'a' && 'href' in nexttag['tags']){
              this.processLink(rawData,nexttagindex,section,null)
            }else if(nexttag['type'] == 'ul'){
              let subsection = rawData.substring(tagendindex+1,nexttagindex)
              let atagbegin = rawData.indexOf("<",rawData.indexOf("<",nexttagindex+1)+1)
              this.processLink(rawData,atagbegin,section,subsection)
            }
  ...
  listTags(s){
    ...
    while (index < s.length){
      ...
      if(nextindex != s.length){
        endindex = matchTag(nextindex)
        tagList.push(s.substring(index,endindex))
        index = endindex
        oldindex = endindex
      }
    }
