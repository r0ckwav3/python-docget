...
export default {
  ...
  getPage(url,callback){
    ...
    let targetID = url.substring(url.indexOf("#")+1,url.length)
    ...
    https.get(docsurlprefix+url ,(res) => {
      ...
      res.on('end', () => {
        let dtindex = rawData.indexOf(`<dt id=\"${targetID}\">`)
        if (dtindex!=-1){
        ...
        }else{ // special case like compound statements
          let idindex = rawData.indexOf(`id=\"${targetID}\"`)
          while(rawData[idindex]!="<"){idindex--;}
          let index = idindex
          ...
          while(flag){
            ...
            if(parsedtag['type'][0] == '/'){
              depth--
              if(depth<0){
                result = rawData.substring(idindex,beginindex)
                flag = false
              }
            }else{
              depth++
