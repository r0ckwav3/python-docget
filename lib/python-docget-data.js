      ...
      res.on('end', () => {
        ...
        while(flag){
          ...
          if(parsedtag['type'] == 'h2' && 'id' in parsedtag['tags']){
            ...
            tempindex = endindex
            innertags = this.listTags(this.tagContents(rawData.substring(beginindex,endindex)))
            ...
            //   }
            //   if(!subsection){
            //     subsection = innertags[0][0]
            //   }
            //
            //   for(let i = 1; i<innertags.length; i++){
            //     if(innertags[i][0] == '<'){
            //       if('href' in this.parseTag(innertags[i])['tags']){
            //         this.processLink(innertags[i],0,section,subsection)
            //       }
            //     }
            //   }
            // }
